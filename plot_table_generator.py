# plot_table_generator.py (Modified for improved exception logging)
# -*- coding: utf-8 -*-
# --- Imports ---
import requests
import pandas as pd
...
try:
    ...
    _fallback_logger_plot = logging.getLogger('plot_gen_fallback')
    def get_site_thresholds(site_id, logger=_fallback_logger_plot): logger.error("threshold_manager_dash import failed"); return None
    def load_thresholds(path, logger=_fallback_logger_plot): logger.error("threshold_manager_dash import failed"); return None
    thresholds_df_global = None
    THRESHOLDS_CSV_PATH = None # Indicate path is unknown
...
def add_gradient_buffer(fig: go.Figure, dates: pd.Series, mean_value: float, buffer: float,
    ...
    except Exception as e_grad: # Catch errors specific to this function
        # --- MODIFICATION: Ensure traceback is logged ---
        logger.error("Error adding gradient buffer", exc_info=True)
...
def apply_flagging(df: pd.DataFrame, thresholds: Optional[Dict[str, Any]], logger: logging.Logger) -> pd.DataFrame:
    ...
    # --- MODIFICATION: Wrap main logic in try-except for robustness ---
    try:
        ...
        return df_processed
         ...
         # --- MODIFICATION: Log traceback for any error during flagging ---
         logger.error("Unexpected error during apply_flagging", exc_info=True)
         # Return original DataFrame or partially processed one? Returning original for safety.
         return df # Return the original unmodified DataFrame
...
def generate_plot_for_site(
    ...
    # --- MODIFICATION: Wrap entire function in try-except for robustness ---
    try:
        ...
        if thresholds_override is None:
            ...
            if (thresholds_df_global is None or thresholds_df_global.empty):
                if THRESHOLDS_CSV_PATH is None or load_thresholds(THRESHOLDS_CSV_PATH, logger) is None: # Attempt load again
                    err = f"CRITICAL ERROR: Threshold load FAILED. Path '{THRESHOLDS_CSV_PATH}' not configured or file could not be loaded/is empty. Cannot proceed."
                    logger.error(err)
                    return None, None, err, station_name, actual_start_date_str, actual_end_date_str, units, None, None
        ...
        if thresholds_override is not None and isinstance(thresholds_override, dict):
            ...
            site_thresholds = get_site_thresholds(site_id, logger) # Call function

        # Check if thresholds were successfully determined
        if site_thresholds is None: # Checks if get_site_thresholds returned None (error or not found)
            err = (f"Error: Thresholds could not be loaded for SiteID {site_id}. Cannot generate plot or apply flags correctly.")
            logger.error(err)
            return None, None, err, station_name, actual_start_date_str, actual_end_date_str, units, None, None
        elif not site_thresholds: # Check if the dictionary is empty (site found but maybe no threshold data)
            logger.warning(f"No specific threshold values found for SiteID {site_id}. Proceeding without threshold-based flagging/plotting.")
            site_thresholds = {} # Ensure it's an empty dict downstream
        else:
        ...
        # --- Handle API/Processing Errors ---
            ...
            err = f"API request timed out (>45s) for SiteID {site_id}."
            logger.error(f"{err} URL: {api_url}", exc_info=True)
            return None, None, err, station_name, start_date_str_requested or "", end_date_str_requested or "", units, site_thresholds, None
        except requests.exceptions.HTTPError as e_http:
            # --- MODIFICATION: Log traceback ---
            err = f"API HTTP Error for SiteID {site_id}: {e_http}. Status Code: {e_http.response.status_code}."
            logger.error(f"{err} URL: {api_url}", exc_info=True)
            return None, None, err, station_name, start_date_str_requested or "", end_date_str_requested or "", units, site_thresholds, None
        except requests.exceptions.RequestException as e_req:
            # Traceback already included via exc_info=True in original code
            err = f"API Request Error fetching data for SiteID {site_id}: {e_req}."
            logger.error(f"{err} URL: {api_url}", exc_info=True)
            return None, None, err, station_name, start_date_str_requested or "", end_date_str_requested or "", units, site_thresholds, None
            ...
            err = (f"JSON Decode or Value Error processing data for SiteID {site_id}. Response snippet: '{response_text_snippet}...'.")
            logger.error(f"{err} URL: {api_url}", exc_info=True) # Log traceback
            return None, None, err, station_name, start_date_str_requested or "", end_date_str_requested or "", units, site_thresholds, None
        # Removed outer Exception catch here as it's handled by the function's main try-except
        ...
        if 'Date' in df_processed.columns:
            try:
                try:
                    ...
                    qualifier_datetime_str = now_dt_slc.strftime('%Y-%m-%d %H:%M %Z%z')
                except ImportError:
                    logger.warning("pytz library not found. Using naive local time for September qualifier timestamp.")
                    now_dt = datetime.now()
                    qualifier_datetime_str = now_dt.strftime('%Y-%m-%d %H:%M')
                ...
                if num_sept_rows > 0:
                    ...
                    for index in indices_to_update:
                        df_processed.at[index, 'Qualifiers'] = september_qualifier.copy()
            ...
            except AttributeError as e_attr: # Catch specific error if 'Date' isn't datetime
                logger.error(f"Error accessing '.dt' accessor, likely 'Date' column is not datetime type", exc_info=True) # Log traceback
            except Exception as e_qual: # Catch other errors during qualifier logic
                # --- MODIFICATION: Ensure traceback is logged ---
                logger.error("An unexpected error occurred during September qualifier application", exc_info=True)
        else:
        ...
        if 'Date' in df_processed.columns and not df_processed.empty:
            ...
            if pd.notna(latest_date):
                ...
                df_processed['ReviewStatus'] = 'Reviewed'
        ...
        def format_qualifier_hover(q_dict):
            ...
            except Exception as e_hover: # Catch errors during formatting
                # --- MODIFICATION: Ensure traceback is logged ---
                logger.error(f"Error formatting qualifier hover text for dict {q_dict}", exc_info=True)
        ...
        df_processed['line_hovertext'] = base_hover_series + qualifier_hover_series + "<extra></extra>"
        ...
        else: df_processed['HasQualifier'] = False

        df_processed['ReviewStatus'] = df_processed.get('ReviewStatus', 'Unknown')
        ...
        mask_unknown = (df_processed['ReviewStatus'] == 'Unknown')
        ...
        df_processed['D_Unknown'] = df_processed['DISCHARGE'].where(mask_unknown)
        ...
        fig.add_trace(go.Scatter(x=df_processed['Date'], y=df_processed['D_Rev_NoQual'], mode='lines', name='Discharge', line=dict(color=COLOR_NORMAL, width=1.5), connectgaps=False, showlegend=True, legendgroup="discharge", hovertext=df_processed['line_hovertext'].where(mask_rev_noqual), hoverinfo='text'))
        fig.add_trace(go.Scatter(x=df_processed['Date'], y=df_processed['D_Raw_NoQual'], mode='lines', name='Discharge (Raw)', line=dict(color=COLOR_NORMAL, width=1.5), connectgaps=False, showlegend=False, legendgroup="discharge", hovertext=df_processed['line_hovertext'].where(mask_raw_noqual), hoverinfo='text'))
        fig.add_trace(go.Scatter(x=df_processed['Date'], y=df_processed['D_Rev_Qual'], mode='lines', name='Discharge (Qualified)', line=dict(color=COLOR_QUALIFIED, width=1.5), connectgaps=False, showlegend=True, legendgroup="discharge_qual", hovertext=df_processed['line_hovertext'].where(mask_rev_qual), hoverinfo='text'))
        fig.add_trace(go.Scatter(x=df_processed['Date'], y=df_processed['D_Raw_Qual'], mode='lines', name='Discharge (Raw Qualified)', line=dict(color=COLOR_QUALIFIED, width=1.5), connectgaps=False, showlegend=False, legendgroup="discharge_qual", hovertext=df_processed['line_hovertext'].where(mask_raw_qual), hoverinfo='text'))
        fig.add_trace(go.Scatter(x=df_processed['Date'], y=df_processed['D_Unknown'], mode='lines', name='Discharge (Unknown Status)', line=dict(color=COLOR_UNKNOWN, width=1.5), connectgaps=False, showlegend=True, legendgroup="discharge_unknown", hovertext=df_processed['line_hovertext'].where(mask_unknown), hoverinfo='text'))
        ...
        for flag_col, (color, label) in flag_plot_info.items():
            if flag_col in df_processed.columns and df_processed[flag_col].any():
                subset = df_processed.loc[df_processed[flag_col]].copy()
                if not subset.empty:
                    fig.add_trace(go.Scatter(
                        x=subset['Date'], y=subset['DISCHARGE'], mode='markers',
                        marker=dict(color=color, size=7, symbol='circle'), name=label,
                        meta=[label] * len(subset), customdata=subset.index,
                        hovertemplate=hover_tmpl_flags, showlegend=True
                    ))
        ...
        if not df_processed.empty and 'Date' in df_processed.columns:
            ...
            if pd.notna(min_plot_dt) and pd.notna(max_plot_dt):
                ...
                if pd.notna(min_val_thresh) and min_val_thresh > 0:
                    fig.add_trace(go.Scatter(x=plot_date_range, y=[min_val_thresh]*2, mode='lines', name=f"Min Threshold ({fmt_min})", line=dict(color="gray", dash="dash", width=1), hoverinfo='skip', showlegend=True))

                if pd.notna(max_val_thresh):
                    max_threshold_label = f"Estimated Max Capacity ({fmt_max})"
                    fig.add_trace(go.Scatter(x=plot_date_range, y=[max_val_thresh]*2, mode='lines', name=max_threshold_label, line=dict(color="purple", dash="dash", width=1.5), hoverinfo='skip', showlegend=True))
                    if max_val_thresh > 0 and len(df_processed['Date']) >= 2:
                        ...
                        add_gradient_buffer(fig, df_processed['Date'], max_val_thresh, buffer_amount, BUFFER_START_COLOR_RGBA, BUFFER_END_COLOR_RGBA, BUFFER_NUM_BANDS, logger)
        ...
        if review_periods:
            ...
            for i, (start_time, end_time, status) in enumerate(review_periods):
                ...
                if end_time == start_time: # Single-day period
                ...
                elif i == len(review_periods) - 1:
                    period_boundaries.append(end_time)

        # --- Add Status Bar Traces ---
        status_header_text = "<b>Data Quality Status</b><br><i>Last year shown as 'Raw' (example)</i>"
        fig.add_trace(go.Scatter(mode='markers', x=[None], y=[None], marker=dict(opacity=0, size=0), name=status_header_text, showlegend=True, hoverinfo='skip'))

        text_font_reviewed_raw = dict(color='white', size=PROGRESS_BAR_TEXT_SIZE)
        text_font_unknown = dict(color='black', size=PROGRESS_BAR_TEXT_SIZE)

        if reviewed_bar_x: fig.add_trace(go.Bar(x=reviewed_bar_x, y=[REVIEW_BAR_Y_VALUE] * len(reviewed_bar_x), width=reviewed_bar_widths, base=0, marker_color=REVIEW_BAR_COLORS['Reviewed'], name='Reviewed', hovertext=reviewed_bar_hover, hoverinfo='text', text=reviewed_bar_text, textposition='inside', insidetextanchor='middle', textfont=text_font_reviewed_raw, yaxis='y2', showlegend=True))
        if raw_bar_x: fig.add_trace(go.Bar(x=raw_bar_x, y=[REVIEW_BAR_Y_VALUE] * len(raw_bar_x), width=raw_bar_widths, base=0, marker_color=REVIEW_BAR_COLORS['Raw'], name='Raw', hovertext=raw_bar_hover, hoverinfo='text', text=raw_bar_text, textposition='inside', insidetextanchor='middle', textfont=text_font_reviewed_raw, yaxis='y2', showlegend=True))
        if unknown_bar_x: fig.add_trace(go.Bar(x=unknown_bar_x, y=[REVIEW_BAR_Y_VALUE] * len(unknown_bar_x), width=unknown_bar_widths, base=0, marker_color=REVIEW_BAR_COLORS['Unknown'], name='Unknown Status', hovertext=unknown_bar_hover, hoverinfo='text', text=unknown_bar_text, textposition='inside', insidetextanchor='middle', textfont=text_font_unknown, yaxis='y2', showlegend=True))
        ...
        if len(period_boundaries) > 2:
            ...
            review_dividing_lines = [
                go.layout.Shape(type='line', xref='x', yref='y2 domain', x0=t, x1=t, y0=0, y1=1, line=dict(color='white', width=2), layer='above')
                for t in internal_boundaries if pd.notna(t)
            ]
        ...
        main_plot_domain_start = progress_bar_height + 0.04 # Increased gap
        ...
        fig.update_layout(
            ...
            xaxis=dict(title_text="", tickfont_size=24, showline=False, zeroline=True, zerolinewidth=1.5, zerolinecolor='darkgrey'),
            yaxis=dict(title_text=f"Discharge ({units})", title_font_size=28, tickfont_size=24, showline=False, zeroline=True, zerolinewidth=1.5, zerolinecolor='darkgrey', domain=[main_plot_domain_start, 1.0]),
            ...
            showlegend=True,
            legend=dict(yanchor="top", y=0.98, xanchor="left", x=1.01, bgcolor="rgba(255,255,255,0.8)", bordercolor="LightGrey", borderwidth=1, font_size=22, tracegroupgap=10, title=dict(text=legend_main_title, font=dict(size=24)), itemclick='toggle', itemdoubleclick='toggle'),
            template="plotly_white",
            margin=dict(t=60, r=250, b=40, l=80),
            height=650,
            hovermode='closest',
        ...
        else: log_end_message += " Processed DataFrame is empty or None."
        ...
        logger.info(f"Thresholds Used: {site_thresholds}")
    ...
    # --- MODIFICATION: Catchall exception handler for the entire function ---
    except Exception as e_main_plot:
        logger.error(f"CRITICAL FAILURE in generate_plot_for_site for SiteID {site_id}", exc_info=True)
        # Return None for fig/df, provide error message and available metadata
        return None, None, f"Unexpected error generating plot: {e_main_plot}", station_name, actual_start_date_str, actual_end_date_str, units, site_thresholds, None
